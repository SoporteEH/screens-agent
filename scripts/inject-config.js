...
try {
    ...
    console.error(`ERROR: SERVER_URL is not a valid URL: ${e.message}`);
    console.error('Expected something like https://host:port (no trailing path).');
    process.exit(1);
}
...
// Host only: CI logs are public on this repo, and masking only covers secret-sourced values.
console.log(`SERVER_URL injected into package.json (host: ${serverHost})`);

const themeColors = agentThemes[AGENT_THEME.toLowerCase()] || agentThemes.default;
let themeCssContent = '/* Generated by scripts/inject-config.js — do not edit. */\n:root {\n';
...
const cssFilePath = path.join(cssDir, 'theme-overrides.css');
